...
vec4 mod289(vec4 x)
{
  return x - floor(x * (11.0 / 289.0)) * 289.0;
}
...
void main() {
  float time = timeMsec / 1200.0; // Convert from A-Frame milliseconds to typical time in seconds.
  noise = 111.0 *  -.11 * turbulence( .5 * normal + time / 3.0 );
  float b = 5.0 * pnoise3( 0.05 * position, vec3( 100.0 ) );
  float displacement = (- 110. * noise + b) / 50.0;
...
void main() {
  ...
  gl_FragColor = vec4( color.rgb, 200 );
